...
After the workflow and updater files are present on the repository's `main` branch:

1. Open the repository on GitHub.
2. Select **Actions**.
3. Select **Update Unpaywall Open Access Links**.
4. Select **Run workflow** and run it on `main`.
5. Confirm that the workflow commits an updated `data/unpaywall.json`.
...
3. Set the name to `UNPAYWALL_EMAIL`.
4. Enter the preferred contact email as its value.

This secret is optional; the workflow works without it.

## GitHub Pages setting

Keep the existing GitHub Pages branch publishing configuration. The workflow commits only `data/unpaywall.json` to `main`; GitHub Pages then republishes the updated static site normally.

There is no need to switch the Pages source to a separate deployment workflow.
